...
- 입력(클라이언트)
  ...
  |주의사항|얼굴이 사진의 40%이상 차지|귀걸이를 제외한 바탕화면을 투명화(PNG 영상 입력)|
...
> StyleAR Web API를 쥬얼리 쇼핑몰에서 빠르고 간단하게 사용하는 방법을 아래 목차에 따라 설명합니다.

- 목차  
  1. 라이센스 키 발급
  2. 쥬얼리 쇼핑몰 웹 페이지 구성
  3. 쥬얼리 쇼핑몰 웹 서버로 데이터 전송
  4. 딥픽셀 StyleAR Web API 요청
  5. 쥬얼리 쇼핑몰 웹 페이지에서 결과 출력

1. 라이센스 키 발급
   > StyleAR Web API를 사용하기 위해서는 반드시 라이센스 키를 발급받아야 하며, 계약된 기간이 지나면 자동적으로 API사용 접근권한이 해제 됩니다.
...
2. 쥬얼리 쇼핑몰 웹 페이지 구성
   > StyleAR Web API를 사용하기 위한 간단한 웹페이지 입니다. 얼굴 및 귀걸이 영상만 입력받도록 구성되어 있습니다.
...
3. 쥬얼리 쇼핑몰 웹 서버로 데이터 전송
   > 쥬얼리 쇼핑몰 웹 페이지에서 취득된 데이터(얼굴, 귀걸이, 귀걸이크기)를 FORM 형식으로 취합하여 쥬얼리 쇼핑몰 웹 서버로 전송합니다.
...
4. 딥픽셀 서버로 데이터 전송
   > 쥬얼리 쇼핑몰 웹 페이지에서 전송받은 데이터(얼굴, 귀걸이, 귀걸이 크기)를 딥픽셀 서버로 전송할 때에는 반드시 해더에 라이센스 키를 포함하여야 하며, 데이터는 폼에 맞게 변환한 후 딥픽셀 서버로 전송합니다. 딥픽셀 서버에서 응답이 오면 응답메시지에 확인하고, JSON형식으로 변환 후 쥬얼리 쇼핑몰 웹 페이지에 전송합니다.
...
5. 쥬얼리 쇼핑몰 웹 페이지에서 결과 출력
   > 쥬얼리 쇼핑몰 웹 서버에서 전달 받은 데이터(귀걸이 착용 얼굴영상, 메타데이터)를 쥬얼리 쇼핑몰 웹 페이지에 출력 합니다.
